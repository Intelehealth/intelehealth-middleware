...
public class PushController {
	...
	private void pushNotifications(String[][] notifiers,int noOfElements)  {
		...
		String url = "https://training.vikalpindia.org:3004/notification/push";
		...
		try {
			...
			//	int vibrationParams[] = {100,50,100};

				Gson gson = new Gson();

				//JsonElement jsonTree = new Gson().toJsonTree(vibrationParams);
				//String vibrationParamsJson = gson.gson.toJson(vibrationParams);


//				patientDetails.addProperty("title", "New patient "+notifiers[row][1]+ "  has been uploaded");
//				patientDetails.addProperty("body", "Please start giving consultation");
//				patientDetails.addProperty("vibrate", gson.toJson(vibrationParams));
