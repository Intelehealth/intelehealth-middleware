...
public interface ProviderDMO {
    ...
    @Select("select 	p.uuid as uuid, "
            ...
            + "where	ur.role IN ('Organizational: Nurse', 'Organizational: Doctor', 'Organizational:MCC') "
            + "group by p.uuid , "
            + "		u.uuid , "
            + "		p.identifier, "
            + "        p.person_id, "
    ...
    ArrayList<ProviderDTO> getProviders();
    ...
    ArrayList<ProviderAttributeTypeDTO> getProviderAttributeTypeMaster(@Param("lastchangedtime") String lastpulldatatime);
    ...
    ArrayList<ProviderAttributeDTO> getProviderAttributes(@Param("lastchangedtime") String lastpulldatatime);
    ...
    @Select("select distinct provider.uuid as uuid, users.uuid as useruuid," +
            ...
            " AND user_role.role IN ('Organizational: Nurse', 'Organizational: Doctor', 'Organizational:MCC')" +
            " AND person_name.voided = 0 and person_name.preferred = 1 AND provider.retired = 0		")
    ArrayList<ProviderDTO> getProviders2(@Param("provider_id") int provider_id);
