...
public interface EncounterDMO {
	...
	@Select("select distinct encounter.uuid as uuid,visit.uuid as visituuid,encounter_type.uuid as encounter_type_uuid,encounter.voided from encounter,visit,obs,location,encounter_type where  encounter.visit_id=visit.visit_id  and visit.location_id=location.location_id and obs.encounter_id=encounter.encounter_id  and obs.obs_datetime>=#{lastchangedtime} and location.uuid=#{locationuuid} and encounter.encounter_type=encounter_type.encounter_type_id")
