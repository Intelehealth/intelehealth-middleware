...
public interface PatientDMO {
  ...
  @Select(
      "SELECT distinct person.uuid as uuid,\n"
          ...
          + " person.voided\n"
          + "FROM person\n"
          + "INNER JOIN patient_identifier pi ON person.person_id = pi.patient_id\n"
  ...
  public ArrayList<PatientDTO> getPatients(
          ...
          @Param("openMrsIdentifierTypeName") String openMrsIdentifierTypeName,
...
//Added patientIds to get the patient attributes for the given patientIds
  ...
  @Select(
      "SELECT person.uuid, pi.identifier AS openmrs_id "
          + "FROM person "
          + "INNER JOIN patient_identifier pi ON person.person_id = pi.patient_id AND pi.voided = 0 "
          + "INNER JOIN patient_identifier_type pit1 "
          + " ON pi.identifier_type = pit1.patient_identifier_type_id "
          + " AND pit1.name = #{openMrsIdentifierTypeName} "
          + "WHERE person.uuid = #{uuid} AND person.voided = 0")
