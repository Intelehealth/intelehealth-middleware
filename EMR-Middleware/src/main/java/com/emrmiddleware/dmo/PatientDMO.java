...
public interface PatientDMO {
	...
	@Select("SELECT distinct person.uuid as uuid,\n" +
            ...
            "and person.creator = users.user_id "+
            "and person.voided=0\n" +
    ...
    @Select("select uuid as uuid ,name, ifnull(description, 'NA') as description from person_attribute_type where COALESCE(date_changed,date_created) >= #{lastchangedtime}")
