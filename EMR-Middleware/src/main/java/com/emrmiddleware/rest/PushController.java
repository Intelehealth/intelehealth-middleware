...
public class PushController {
	...
	public Response setData(PushDataDTO pushdatadto, @Context HttpHeaders httpHeaders) {
		...
		try {
			...
			e = pdd.getEncounters();
			//System.out.print("Number of Encounters" + e.size());
			if(e.size() > 0) {
	...
	private void pushNotifications(String[][] notifiers,int noOfElements)  {
		...
		try {
			...
			for (int row = 0; row < noOfElements; row++) {
				...
				//System.out.print(mainPacket);

				DataOutputStream wr = new DataOutputStream(con.getOutputStream());
				wr.writeBytes(String.valueOf(mainPacket));

				wr.flush();
					//System.out.print("Response for notification push to URL "+ url + " is : "+con.getResponseCode()+"--"+con.getResponseMessage());

				wr.close();
			//	//System.out.print("Response for notification push: "+con.getResponseCode()+"--"+con.getResponseMessage());


			}
		}catch(Exception e) {
			//System.out.print("Exception caught in pushnotification ");
			e.printStackTrace();
	...
	public String findProvider(String providerUID) {
		...
		try {
			...
			//System.out.print("Exception caught in findprovider ");
			e.printStackTrace();
		}
	...
	public int checkVisit(String visitUid, String patientUid) {
		...
		try {
			...
			//System.out.print("Exception caught in checkvisit ");
			e.printStackTrace();
		}
	...
	public String getPatientName(String patientUID) {
		...
		try {
			...
			//System.out.print("Exception caught in getPatientName ");
			e.printStackTrace();
		}
